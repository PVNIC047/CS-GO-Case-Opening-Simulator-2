  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8 max-w-7xl mx-auto gap-4">
        <h2 className="text-xl sm:text-3xl font-bold text-white uppercase tracking-wider">Your Inventory</h2>
        <div className="flex items-center gap-4">
          ...
          <div className="text-gray-400 text-xs sm:text-sm">Total: {items.length}</div>
        </div>
      </div>
        ...
        {sortedItems.map((invItem) => (
          <div key={invItem.id} className="relative group">
            ...
            <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center rounded-md backdrop-blur-sm p-2 gap-2">
              ...
              >
                Sell ${calculateItemPrice(invItem).toFixed(2)}
              </button>
            </div>
          </div>
        ))}
